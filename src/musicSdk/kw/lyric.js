...
export default {
  ...
  //   return requestObj
  // },
  // 沙箱适配：原 newlyric.lrc 返回 gb18030 压缩数据（QuickJS 无 iconv），
  // 改走 m.kuwo.cn 的 UTF-8 JSON 端点 songinfoandlrc，无需 GBK/inflate。
  formatLrcTime(t) {
    const v = parseFloat(t) || 0
    const m = Math.floor(v / 60)
    let s = (v % 60).toFixed(2)
    if (parseFloat(s) < 10) s = '0' + s
    return `[${m < 10 ? '0' + m : m}:${s}]`
  },
  getLyric(musicInfo) {
    const requestObj = httpFetch(`http://m.kuwo.cn/newh5/singles/songinfoandlrc?musicId=${musicInfo.songmid}`)
    requestObj.promise = requestObj.promise.then(({ body }) => {
      const data = body && body.data
      if (!data || !data.lrclist || !data.lrclist.length) return Promise.reject(new Error('Get lyric failed'))
      const lyric = data.lrclist
        .map(l => `${this.formatLrcTime(l.time)}${decodeName(l.lineLyric || '')}`)
        .join('\n')
      if (!lyric) return Promise.reject(new Error('Get lyric failed'))
